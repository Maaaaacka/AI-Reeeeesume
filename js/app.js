(function() {
  ...
  const hasHardcodedColors = (html) => /#[0-9a-f]{3,8}|rgba?\([^)]+\)|hsla?\([^)]+\)/gi.test(html);
  ...
  const aiService = {
    async generateFirstQuestion(resumeData) {
      ...
      try { return await utils.callAPI([{ role: 'user', content: prompt }], 0.7); }
      catch (e) { return '可以介绍一下你的工作经历吗？从最近的开始。'; }
    },
    async processUserAnswer(resumeData, messages) {
      ...
      try {
        const aiReply = await utils.callAPI([{ role: 'system', content: systemPrompt }, ...history], 0.5, true);
        return JSON.parse(aiReply);
      } catch (error) { throw new Error('处理回答时出错'); }
    },
    async generateTemplate(resumeData, customPrompt = '', primaryColor = '#6C8EB2', fontFamily = 'system') {
      const fontMap = { system: '-apple-system, BlinkMacSystemFont, "Helvetica Neue", Helvetica, Arial, sans-serif', sans: 'Arial, Helvetica, sans-serif', serif: 'Georgia, "Times New Roman", serif', mono: '"Courier New", monospace' };
      ...
      try { return await utils.callAPI([{ role: 'user', content: prompt }], 0.6, false); }
      catch (e) { throw new Error('生成模板失败，请检查网络或API配置'); }
    },
    async polishContent(resumeData) {
      ...
      try { return JSON.parse(await utils.callAPI([{ role: 'user', content: prompt }], 0.5, true)); }
      catch (e) { throw new Error('润色内容失败'); }
    }
  };

  const app = createApp({
    components: { ColorWheelPicker: window.ColorWheelPicker, ProjectEditor: window.ProjectEditor },
    setup() {
      ...
      const presetDescs = ['经典卡片布局（简洁稳重）','圆角卡片布局（留白较多）','现代感布局（强调科技感）'];
      const randomPreset = () => { templatePrompt.value = presetDescs[Math.floor(Math.random() * presetDescs.length)]; };
      const setPreset = (index) => { templatePrompt.value = presetDescs[index]; };

      const showToast = (message, type = 'text') => {
        if (window.vant && window.vant.showToast) window.vant.showToast({ message, type });
        else alert(`[${type}] ${message}`);
      };

      const selectedFontLabel = computed(() => FONT_OPTIONS.find(f => f.value === customFont.value)?.label || '系统默认');
      ...
      const fillTemplateWithData = (templateHtml) => {
        ...
        html = html.replace(/var\(--primary-color\)/g, customColor.value).replace(/var\(--font-family\)/g, fontFamily);
        ...
        resume.skills.forEach((skill, index) => {
          if (skillItems[index]) skillItems[index].textContent = typeof skill === 'string' ? skill : (skill.name || '');
        });
        ...
        resume.projects.forEach((proj, index) => {
          if (projItems[index]) {
            ...
            if (linkEl) { linkEl.href = proj.url || '#'; linkEl.textContent = '查看项目'; }
          }
        });
        return tempDiv.innerHTML;
      };

      const refreshPreview = () => { if (currentTemplate.value) polishedHTML.value = fillTemplateWithData(currentTemplate.value); };
      watch(resume, () => { if (currentStep.value === 3 && currentTemplate.value) refreshPreview(); }, { deep: true });
      watch(messages, async () => { await nextTick(); if (chatBox.value) chatBox.value.scrollTop = chatBox.value.scrollHeight; });

      const updatePersonalFromForm = () => { resume.personal = { ...basicForm }; };
      const submitBasic = () => {
        if (!basicForm.name || !basicForm.jobTitle) { showToast('请填写姓名和求职意向', 'fail'); return; }
      ...
      const startAIConversation = async () => {
        ...
        } finally { isWaitingAI.value = false; }
      };
      const sendAnswer = async () => {
        ...
        try {
          ...
          if (result.next_question) messages.value.push({ role: 'ai', content: result.next_question });
          else messages.value.push({ role: 'ai', content: '信息收集完成！点击"下一步"生成简历模板。' });
        } catch (error) {
      ...
      const openEditPanel = () => { manualJSON.value = JSON.stringify(resume, null, 2); showEditPanel.value = true; };
      ...
      const applyManualEditOnly = () => {
        ...
        } catch (e) { showToast('JSON格式错误', 'fail'); }
      };
      const applyAndPolishContent = async () => {
        ...
        } catch (e) { showToast('操作失败', 'fail'); } finally { isWaitingAI.value = false; }
      };
      ...
      const saveDraft = () => {
        try {
          const draft = { currentStep: currentStep.value, basicForm: { ...basicForm }, messages: messages.value, resume: { ...resume }, currentTemplate: currentTemplate.value, customFont: customFont.value, customColor: customColor.value, templatePrompt: templatePrompt.value };
          localStorage.setItem(DRAFT_KEY, JSON.stringify(draft));
          showToast('草稿已保存', 'success');
        } catch (e) { showToast('保存失败：' + e.message, 'fail'); }
      };
      const loadDraft = () => {
        try {
          const saved = localStorage.getItem(DRAFT_KEY);
          if (!saved) { showToast('无保存的草稿', 'fail'); return; }
        ...
        } catch (e) { showToast('草稿数据损坏或加载失败', 'fail'); }
      };

      const exportWord = () => {
        if (!resume.personal.name) { showToast('请先填写基本信息', 'fail'); return; }
        if (!polishedHTML.value) { showToast('暂无预览内容', 'fail'); return; }
        const buildExportHTML = () => {
          ...
          const name = resume.personal.name || '', jobTitle = resume.personal.jobTitle || '', email = resume.personal.email || '', phone = resume.personal.phone || '';
          let html = `<!DOCTYPE html>\n<html>\n<head>\n  <meta charset="UTF-8">\n  <style>\n    @page { size: A4; margin: 2.5cm; }\n    body { font-family: ${fontFamily}; font-size: 12pt; line-height: 1.4; color: #1e293b; }\n    h1 { font-size: 28pt; font-weight: bold; color: ${primaryColor}; text-align: center; margin-bottom: 10px; border-bottom: 2px solid ${primaryColor}; padding-bottom: 10px; }\n    .contact-info { text-align: center; font-size: 14pt; color: #4a5568; margin-bottom: 20px; }\n    h2 { font-size: 18pt; font-weight: bold; color: ${primaryColor}; border-bottom: 1px solid ${primaryColor}; padding-bottom: 5px; margin-top: 25px; margin-bottom: 15px; }\n    .experience-item, .education-item { margin-bottom: 20px; }\n    .item-header { font-weight: bold; font-size: 14pt; }\n    .item-date { float: right; color: #718096; font-style: italic; }\n    .item-desc { margin-top: 5px; margin-left: 20px; }\n    .skills { display: flex; flex-wrap: wrap; gap: 10px; }\n    .skill-tag { background-color: ${primaryColor}20; color: ${primaryColor}; padding: 5px 12px; border-radius: 20px; font-size: 11pt; border: 1px solid ${primaryColor}40; }\n    .projects-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-top: 12px; }\n    .project-card { background: #f9f9f9; border: 1px solid #e0e0e0; border-radius: 12px; padding: 16px; }\n    .project-name { font-weight: 600; color: ${primaryColor}; margin-bottom: 4px; }\n    .project-desc { font-size: 12px; color: #555; margin-bottom: 8px; }\n    .project-link { color: ${primaryColor}; text-decoration: underline; font-size: 12px; }\n  </style>\n</head>\n<body>\n  <h1>${escapeHtml(name)}</h1>\n  <div class="contact-info">${escapeHtml(jobTitle)} | ${escapeHtml(email)} | ${escapeHtml(phone)}</div>`;
          if (resume.summary) html += `<h2>摘要</h2><p>${escapeHtml(resume.summary)}</p>`;
          if (resume.experience?.length) {
            html += `<h2>工作经历</h2>`;
            resume.experience.forEach(exp => {
              html += `<div class="experience-item"><div class="item-header">${escapeHtml(exp.title || '')} @ ${escapeHtml(exp.company || '')} <span class="item-date">${escapeHtml(exp.date || '')}</span></div>`;
              if (exp.description) {
                let descText = Array.isArray(exp.description) ? exp.description.map(i => escapeHtml(i)).join('<br/>') : (typeof exp.description === 'object' ? escapeHtml(JSON.stringify(exp.description, null, 2)) : escapeHtml(String(exp.description)));
          ...
          if (resume.education?.length) {
            html += `<h2>教育背景</h2>`;
            resume.education.forEach(edu => html += `<div class="education-item"><div class="item-header">${escapeHtml(edu.degree || '')} @ ${escapeHtml(edu.school || '')} <span class="item-date">${escapeHtml(edu.date || '')}</span></div></div>`);
          }
          if (resume.skills?.length) {
            html += `<h2>技能</h2><div class="skills">`;
            resume.skills.forEach(skill => { const sn = typeof skill === 'string' ? skill : (skill.name || ''); html += `<span class="skill-tag">${escapeHtml(sn)}</span>`; });
            html += `</div>`;
          }
          if (resume.projects?.length) {
            html += `<h2>作品/项目</h2><div class="projects-grid">`;
            resume.projects.forEach(proj => {
              html += `<div class="project-card"><div class="project-name">${escapeHtml(proj.name || '')}</div>`;
              if (proj.description) html += `<div class="project-desc">${escapeHtml(proj.description)}</div>`;
              if (proj.url) html += `<a href="${escapeHtml(proj.url)}" class="project-link">查看项目</a>`;
              html += `</div>`;
            });
            html += `</div>`;
          }
      ...
      const progressWidth = computed(() => ((currentStep.value - 1) / 3 * 100) + '%');
      const handleClickOutside = (event) => { if (fontSelectorOpen.value && !event.target.closest('.font-selector')) fontSelectorOpen.value = false; };

      return {
        currentStep, basicForm, messages, userInput, isWaitingAI, polishedHTML, chatBox,
        customFont, customColor, showEditPanel, manualJSON, templatePrompt,
        presetDescs, PRESET_COLORS, FONT_OPTIONS, selectedFontLabel, fontSelectorOpen, progressWidth,
        submitBasic, sendAnswer, goToPreview, exportWord, saveDraft, loadDraft,
        openEditPanel, closeEditPanel, applyManualEditOnly, applyAndPolishContent, applyAndChangeTemplate,
    ...
    template: `
      <div class="app-container">
        ...
        <div class="status-bar">
          ...
          <div class="draft-actions"><button @click="saveDraft">保存</button><button @click="loadDraft">载入</button></div>
        </div>
        ...
        <div class="content">
          ...
          <div v-if="currentStep === 1" class="section">
            ...
            <div class="action-buttons"><button class="action-btn primary" @click="submitBasic">开始AI收集</button></div>
          </div>
          <!-- 步骤2 -->
          <div v-else-if="currentStep === 2" class="section">
            <div class="card">
              <details><summary style="color: var(--text-light);">当前简历</summary><div class="summary-block"><pre>{{ JSON.stringify(resume, null, 2) }}</pre></div></details>
            </div>
            <div class="chat-container">
              <div class="chat-messages" ref="chatBox">
                ...
                <div v-if="isWaitingAI" class="message ai"><div class="message-bubble">⏳ AI思考中...</div></div>
              </div>
            ...
            <div class="action-buttons"><button class="action-btn secondary" @click="goToPreview">直接预览</button></div>
          </div>
          <!-- 步骤3 -->
            ...
            <div class="color-section">
              <div class="color-picker-container">
                ...
                <div class="preset-colors" style="margin-top: 16px;">
                  <div v-for="color in PRESET_COLORS" class="color-dot" :style="{ backgroundColor: color }" :class="{ active: customColor === color }" @click="customColor = color"></div>
                </div>
              </div>
            </div>
              ...
              <div class="setting-item">
                ...
                <div class="font-selector">
                  <div class="font-selector-trigger" :class="{ active: fontSelectorOpen }" @click.stop="fontSelectorOpen = !fontSelectorOpen">
                    <span>{{ selectedFontLabel }}</span><span class="font-selector-arrow" :class="{ open: fontSelectorOpen }">▼</span>
                  </div>
                  <div class="font-selector-dropdown" :class="{ show: fontSelectorOpen }">
                    <div v-for="font in FONT_OPTIONS" class="font-option" :class="{ selected: customFont === font.value }" @click="customFont = font.value">{{ font.label }}</div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card" style="padding: 16px;"><ProjectEditor v-model="resume.projects" /></div>
            <div class="preview-section"><div class="preview-content" v-html="polishedHTML"></div></div>
          ...
          <div v-else-if="currentStep === 4" class="section">
            <div class="preview-section"><div class="preview-readonly" v-html="polishedHTML"></div></div>
            <div class="action-buttons"><button class="action-btn primary" @click="exportWord">导出Word</button></div>
            <button class="back-link" @click="currentStep = 3">← 返回修改</button>
          </div>
        </div>
        <!-- 编辑面板 -->
        <div class="edit-panel" :class="{ open: showEditPanel }">
          <div class="edit-panel-header"><h3>编辑JSON</h3><button class="edit-panel-close" @click="closeEditPanel">✕</button></div>
